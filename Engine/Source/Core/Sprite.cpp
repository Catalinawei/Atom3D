...
btVector3 Sprite::GetHalfExtents(std::vector<glm::vec3>& vertices) {
	...
	btVector3 size = maxVec - minVec;

	if (size.x() < 0.001f) size.setX(0.001f);
	if (size.y() < 0.001f) size.setY(0.001f);
	if (size.z() < 0.001f) size.setZ(0.001f);
