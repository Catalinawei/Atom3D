...
void Game::Initiallize() {
	...
	{
		auto m_lightingShader = std::make_unique<Shader>("Assets/Shaders/LightingVertexShader.glsl",
			"Assets/Shaders/LightingFragmentShader.glsl");
		auto m_lightingCubeShader = std::make_unique<Shader>("Assets/Shaders/LightingVertexShader.glsl", 
			"Assets/Shaders/LightingCubeFragmentShader.glsl");
		...
		m_mainScene->Initiallize("Assets/Scenes/Scene.sc", 
	...
	while (!glfwWindowShouldClose(m_window.get())) {
		...
		if (initializationThread.valid()) {
			...
			if (status == std::future_status::ready) {
				...
				std::cout << "Audio prepared!!11!!!!1111\n";
				soundGood = true;
			}
		}
		...
		if (!m_mainScene)
			std::cerr << "main scene is nullptr\n";
		if (!m_mainRenderer)
			std::cerr << "MAIN RENDERER IS NULLPTR\n";
